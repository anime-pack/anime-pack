...
import { LoginForm, SignupForm } from '@/components/layout/Login';
...
const randomBgUrl = computed(() => {
    const randomIndex = Math.floor(Math.random() * loginBgs.length);
    return `url("${loginBgs[randomIndex]}")`;
});
</script>

<template>
    <main class="min-h-screen w-full overflow-hidden" role="main">
        <div :style="{ backgroundImage: randomBgUrl }"
            class="relative h-full min-h-screen w-full bg-cover bg-center bg-no-repeat">
            <!-- Overlay for better contrast -->
            <div class="absolute inset-0 bg-background/60 backdrop-blur-sm"></div>

            <!-- Content container -->
            <section class="relative flex min-h-screen w-full items-center justify-center p-4 sm:p-8">
                <div class="w-full max-w-md rounded-lg bg-card/80 p-6 shadow-lg backdrop-blur-md 
                transition-all duration-300 hover:bg-card/90 sm:p-8" role="region"
                aria-label="Login form container">
                <div class="flex flex-row justify-end">
                    <RouterLink to="/" class=" flex flex-row items-center gap-1 py-1 hover:underline underline-offset-2">
                        Login as Guest
                    ...
                    <header class="mb-8 text-center">
                        <div class="mx-auto mb-4 flex size-16 items-center justify-center rounded-xl 
                            bg-[#f9e7a9] text-primary-foreground shadow-md transition-transform 
                            hover:scale-105">
                            <img src="@/assets/app_icon.svg" alt="App Icon" class="h-16 w-16 mx-auto mb-2" />
                        </div>
                        <h1 class="text-2xl font-semibold text-foreground">Anime Pack</h1>
                    </header>

                    <!-- Forms -->
                    <LoginForm @toggleSignup="() => { isSignup = !isSignup }" v-if="!isSignup"
                        class="animate-in fade-in-50 duration-500" />
                    <SignupForm @toggleSignup="() => { isSignup = !isSignup }" v-else
                        class="animate-in fade-in-50 duration-500" />
                </div>
            </section>
        </div>
